...
# def cleaning_ocr_text(text: str) -> str:
#     """Membersihkan teks hasil OCR dari noise, karakter aneh, dan kata tidak baku."""
#     text = fix_text(text)
#     text = unidecode(text)
#     text = re.sub(r'[_\-–—=•·]+', ' ', text)
#     text = re.sub(r'([a-z])\s*-\s*([a-z])', r'\1\2', text)
...
#     clean_words = []
#     for w in words:
#         if re.fullmatch(r'[A-Z]{2,}', w):  # kapital semua
#             continue
#         if len(re.findall(r'[bcdfghjklmnpqrstvwxyz]', w.lower())) > len(w) * 0.8:
#             continue
#         if len(w) <= 2 or w.isdigit():
#             continue
...
#         # --- Fallback jika hasilnya kacau (terlalu banyak huruf acak) ---
#         if len(re.findall(r'[a-zA-Z]', text)) > 0 and len(text) / (len(re.findall(r'[a-zA-Z]', text)) + 1) < 1.5:
#             return text
#         if len(text) < 10 or re.search(r'[bcdfghjklmnpqrstvwxyz]{6,}', text):
#             # Coba mode lain (lebih fokus per baris)
#             alt = pytesseract.image_to_string(gray, lang=lang, config="--psm 7")
#             alt = re.sub(r'\s+', ' ', alt).strip()
#             if len(alt) > len(text):
#                 text = alt
...
#     cached_images = [os.path.join(cache_subdir, f) for f in sorted(os.listdir(cache_subdir)) if f.endswith('.jpg')][:5]
...
#     text = cleaning_ocr_text(text)
#     return {"text": text, "length": len(text), "time_taken": time.time() - start_time}
